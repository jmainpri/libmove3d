...
Graph::Graph(p3d_graph* G, Robot* R)
{
  if (G){
...
void Graph::setName()
{
  _Name =  "graph" +  _Robot->getNbCreatedGraph();
}
...
Node* Graph::insertNode(Node* node)
{
  ...
  this->getGraphStruct()->dist_nodes = p3d_add_node_to_list(node->getNodeStruct(), this->getGraphStruct()->dist_nodes);
  return(node);
}
...
bool Graph::isEdgeInGraph(Node* N1,Node* N2)
{
  ...
  {
    flag = (_Edges[i]->getEdgeStruct()->Ni == N1->getNodeStruct() && _Edges[i]->getEdgeStruct()->Nf == N2->getNodeStruct());
    if (flag)
      return flag;
  }
  return flag;
...
void Graph::addNode(Node* N, double maxDist)
{
  ...
  {
    if((d = _Nodes[i]->getConfiguration()->dist(*N->getConfiguration())) < maxDist)
    {
    shared_ptr<LocalPath> path = shared_ptr<LocalPath>(new LocalPath(_Nodes[i]->getConfiguration(), N->getConfiguration()));
      if(path->getValid())
...
vector<Node**> Graph::isOrphanLinking(Node* N, int* link)
{
  ...
  for (int j = 1; j <= _Graph->ncomp; j = j + 1)
  {
    for (uint i = 0; i < _Nodes.size(); i = i + 1)
    {
      if (_Nodes[i]->getNodeStruct()->numcomp == j)
      {
        if (_Nodes[i]->isLinkable(N, &dist) && (dist < ENV.getDouble(Env::dist) || !ENV.getBool(Env::useDist)))
        {
          *link = *link + 1;
...
bool Graph::linkOrphanLinking(Node* N, p3d_graph* Graph_Pt, void (*fct_draw)(void), int type, int* ADDED, int* nb_fail)
{
  int link =  0;
...
void Graph::createOneOrphanLinking(p3d_graph* Graph_Pt, void (*fct_draw)(void), int type, int* ADDED, int* nb_fail)
{
...
int Graph::createOrphansLinking(int nb_node, int (*fct_stop)(void), void (*fct_draw)(void), int type)
{
  int ADDED = 0;
  int nb_try = 0;
  while((*fct_stop)() && _Nodes.size() < nb_node && nb_try < ENV.getInt(Env::NbTry) && (_Graph->ncomp > 1 || !type))
  {
...
void Graph::createRandConfs(int NMAX, int (*fct_stop)(void), void (*fct_draw)(void)) {
  int inode;
  double tu, ts;
  ...
  shared_ptr<Configuration> Cs = shared_ptr<Configuration>(new Configuration(_Robot, _Robot->getRobotStruct()->ROBOT_POS));
				    ...
				    int distConfigChoice)
{
  ...
  while(nodes) {
    ...
    {
      if(distConfigChoice == MOBILE_FRAME_DIST)
	CurrentDist = p3d_GetSe3DistanceFrames(_Graph->rob, MobFrameRef,
					       nodes->N->RelMobFrame);
      else
	CurrentDist = C->dist(*_NodesTable[nodes->N]->getConfiguration(), distConfigChoice);

      CurrentScore = CurrentDist * (weighted ? p3d_GetNodeWeight(nodes->N) : 1.0);
...
Node* Graph::insertNode(shared_ptr<Configuration> q, Node* expansionNode, double currentCost, double step)
{
...
void Graph::addCycles(Node* node, double step)
{
  ...
  p3d_list_node* listDistNodePt = p3d_listNodeInDist(_Robot->getRobotStruct(),
					     node->getNodeStruct()->comp, node->getNodeStruct(), longStep);
  ...
  while(listDistNodePt) {
    if(!p3d_IsSmallDistInGraph(_Graph, node->getNodeStruct(), listDistNodePt->N, 5, step)) {
      LP = shared_ptr<LocalPath> (new LocalPath(node->getConfiguration(), this->getNode(listDistNodePt->N)->getConfiguration()));
      if(LP->getValid() &&
         this->getNode(listDistNodePt->N)->getConfiguration()->costTestSucceeded(node,step) &&
	 node->getConfiguration()->costTestSucceeded(this->getNode(listDistNodePt->N),step))
...
Node* Graph::insertRrtLinkingNode(shared_ptr<Configuration> q, Node* from, double step)
{
  Node* node = new Node(this, q);
  this->insertNode(node);
  from->connectNodeToCompco(node, step);
