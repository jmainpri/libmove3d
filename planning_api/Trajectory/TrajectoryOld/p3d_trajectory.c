 ...
 * Output FALSE in case of success
 *
 * note the start and end configurations must be equals on the trajectories
 *
 */
  ...
  if (!p3d_equal_config(robotPt, q11, q21))
    {
      ...
      PrintInfo(("Start configurations are not the same in replace trajectory\n"));
      return TRUE;
    }
  ...
  if (!p3d_equal_config(robotPt, q12, q22))
    {
      ...
      p3d_destroy_config(robotPt, q21);
      p3d_destroy_config(robotPt, q22);
      PrintInfo(("Goal configurations are not the same in replace trajectory\n"));
      return TRUE;
    }
  p3d_destroy_config(robotPt, q11);
  p3d_destroy_config(robotPt, q12);
 ...
 */
void p3d_GetRandSuccesConfAlongTraj(p3d_traj* trajPt,
		configPt* qPrevPt, configPt*  qCurrentPt, configPt* qNextPt,
		double* prevDistPt, double* randDistPt, double* nextDistPt) {
...
Get Random Successive Configurations\n"));
 ...
 double step = p3d_random(p3d_get_env_dmax(),
			  p3d_get_env_dmax() * ENV.getDouble(Env::extensionStep) * 5);
