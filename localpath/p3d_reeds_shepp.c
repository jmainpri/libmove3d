				      ...
				      configPt qf, int* ikSol)
{
  ...
  q_f[x_coord] = x_dof;            q_f[y_coord] = y_dof;
  ...
  if(p3d_col_test()){
    PrintInfo(("MP: p3d_rsarm_localplanner (1): Illegal q_init\n"));
  }
  
  p3d_set_robot_config(robotPt, q_f);
  p3d_update_robot_pos();
  if(p3d_col_test()){
    PrintInfo(("MP: p3d_rsarm_localplanner: Illegal q_goal\n"));
  }
  */
