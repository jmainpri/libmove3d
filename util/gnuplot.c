...
/*
  $Id: gnuplot.c,v 1.2 2008/09/16 14:42:53 mgharbi Exp $
  $Author: mgharbi $
  $Date: 2008/09/16 14:42:53 $
  $Revision: 1.2 $
 */
...
/*-------------------------------------------------------------------------*/
/**
  @brief    Switch to log scale x or/and y axes
  @param    h Gnuplot session control handle
  @param    axeScale axe to scale (character string)
  @return   void

  The provided scale is a character string. It must be one of
  the following:
   x
   y
   xy
...
void gnuplot_setlogscale(gnuplot_ctrl * h, char * axeScale) {
  ...
  if (!strcmp(axeScale, "x") || !strcmp(axeScale, "y")) {
    sprintf(cmd, "set nologscale");
    gnuplot_cmd(h, cmd);
    sprintf(cmd, "set logscale %s", axeScale) ;
  } else
    if (!strcmp(axeScale, "xy")) {
      sprintf(cmd, "set logscale") ;
    } else if (!strcmp(axeScale, "none")) {
      sprintf(cmd, "set nologscale");
    } else {
      fprintf(stderr, "warning: unknown requested scale: using default\n");
    }
  gnuplot_cmd(h, cmd);
}
...
/*-------------------------------------------------------------------------*/
/**
  @brief    Change the ranges of x or y axes
  @param    h Gnuplot session control handle
  @param    axeRange axe to change thhe range (character string)
...
void gnuplot_setrange(gnuplot_ctrl * h, char * axeScale, double minRange, double maxRange) {
  ...
  if (!strcmp(axeScale, "x") || !strcmp(axeScale, "y")) {
    sprintf(cmd, "set %srange [%f:%f]", axeScale, minRange, maxRange) ;
  } else {
    fprintf(stderr, "warning: unknown requested range\n");
  }
  gnuplot_cmd(h, cmd);
}
/*-------------------------------------------------------------------------*/
...
void gnuplot_plot_x(
  ...
  sprintf(file,"%s/video/%s.gnuplot", getenv("HOME_MOVE3D"), title);
  handle->curves[handle->nbCurves] = fopen(file, "w") ;
  ...
  close(tmpfd) ;
  fclose(handle->curves[handle->nbCurves - 1]);
  ...
  handle->nplots++ ;
  if (title == NULL) {
    sprintf(line, "%s \"%s\" with %s", cmd, file, handle->pstyle) ;
  } else {
    sprintf(line, "%s \"%s\" title \"%s\" with %s", cmd, file,
            title, handle->pstyle) ;
  }
  return ;
}
