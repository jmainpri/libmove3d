...
Node* TreeExpansionMethod::getExpansionNode(Node* compNode,
	...
	{
			return mGraph->getNode(compNode->getComp()->dist_nodes->N);
	}
	else
	{
		...
		shared_ptr<Configuration> direction, int distance)
{
 ...
 case RANDOM_IN_SHELL_METH:
   ...
   return(mGraph->getNode(hrm_selected_pb_node(mGraph->getGraph(),
					   direction->getP3dConfigPt(),
					   compNode->mN->comp)));

 case RANDOM_NODE_METH:
   return(mGraph->getNode(p3d_RandomNodeFromComp(compNode->mN->comp)));
