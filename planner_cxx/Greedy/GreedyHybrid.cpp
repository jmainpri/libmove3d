...
GreedyHybrid::GreedyHybrid(p3d_graph* G,
		int (*stop_func)(),
		void (*draw_func)()) :
			...
			mGraph(new Graph(G ? G : p3d_create_graph(),
					mRobot,
					new LocalpathFactory(),
					new SamplingAPI(mRobot))),
					mStart(NULL),
					mGoal(NULL) {
	...
	Configuration startConf(mRobot->getRobotPos());
...
GreedyHybrid::~GreedyHybrid(){
/*	delete Expansion;
	delete trajPt;*/
}


int GreedyHybrid::run() {

	cout << "-------------- Start Diffusion ----------------------" << endl;

	Diffusion = new RRT(mGraph->getGraph(),
			fct_stop,
			fct_draw);
	...
	if(traj_exist){
		...
		optimTrj = new CostOptimization(mRobot,p3d_ExtractBestTraj(mGraph->getGraph()));
...
void GreedyHybrid::shortCutLinear() {
...
void GreedyHybrid::optimizeLinear() {
...
void GreedyHybrid::optimizePhaze(){
...
bool GreedyHybrid::checkStopConditions()
{
...
GreedyHybrid* OptPlanner;

bool p3d_RunGreedyHybrid(p3d_graph* GraphPt, int (*fct_stop)(void),
		void (*fct_draw)(void)) {

	cout << endl
	<< "**************************************" << endl
	<< " Beginning of Greedy Hybrid search process" << endl << endl;
	...
	OptPlanner = new GreedyHybrid(GraphPt,fct_stop,fct_draw);
	...
	cout << endl
	<< " End of Greedy Hybrid search process" << endl
	<< "**************************************" << endl << endl;
