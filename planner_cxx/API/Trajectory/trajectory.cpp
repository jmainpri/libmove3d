...
void Trajectory::replaceP3dTraj(p3d_traj* trajPt)
{
	...
	for (uint i = 1; i < nloc; i++)
	{
		...
		localpathPt = mCourbe.at(i)->getLocalpathStruct()->copy(mRobot->getRobotStruct(), 
								  mCourbe.at(i)->getLocalpathStruct());
...
vector< shared_ptr<Configuration> > Trajectory::getVectorOfConfiguration()
{
	...
	vect.push_back( mCourbe[0]->getBegin() );
	
	for (unsigned int i=0; i<mCourbe.size(); i++) 
		vect.push_back( mCourbe[i]->getEnd() );
...
{
	return computeSubPortionCost(mCourbe);
}

double Trajectory::costDeltaAlongTraj()
{
	cout << "Sum of LP cost = " << computeSubPortionCost(mCourbe) << endl;
	Trajectory tmp(*this);
	cout << "Recomputed Sum of LocalPaths = "  << tmp.ReComputeSubPortionCost(tmp.mCourbe) << endl;
	return computeSubPortionIntergralCost(mCourbe);
}
