...
Smoothing::Smoothing() :
m_nbBiased(0), m_nbReallyBiased(0), m_ShortCutBiased(true)
{
	m_Selected.resize(0);
}
...
{	
	...
	if ( ENV.getBool(Env::withMaxIteration) ) 
	{
	...
	if( ENV.getBool(Env::withTimeLimit) )
	{
		if ( m_time > ENV.getDouble(Env::optimTimeLimit) ) 
		{
	...
	if ( ENV.getBool(Env::withGainLimit) ) 
	{
		const int n = 10; 
 ...
 */
bool Smoothing::oneLoopShortCut( double step )
{
	...
	vectConf = get2RandomConf(step,lFirst, lSecond);
