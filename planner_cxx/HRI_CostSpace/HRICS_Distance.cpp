...
//        cout << _Robot->getRobotStruct() << endl;
        ...
        for(int i =0; i<_Humans[j]->getRobotStruct()->no; i++)
        {
            ...
            {
                p3d_col_activate_rob_obj(_Robot->getRobotStruct(),_Humans[j]->getRobotStruct()->o[i]);
                //                cout << "Activate " << _Robot->getName() << " to " << _Humans[j]->getRobotStruct()->o[i]->name << endl;
            }
            else
            {
                p3d_col_deactivate_rob_obj(_Robot->getRobotStruct(),_Humans[j]->getRobotStruct()->o[i]);
            }
        }
    }
}
    ...
    for(int j=0; j<_Humans.size(); j++)
    {
        ...
        for(int i =0; i<_Humans[j]->getRobotStruct()->no; i++)
        {
            if(binary_search(_SafetyZonesBodyId[j].begin(), _SafetyZonesBodyId[j].end(),i))
            {
                //                p3d_col_activate_rob_obj(_Robot->getRobotStruct(),_Humans[j]->getRobotStruct()->o[i]);
                p3d_col_deactivate_rob_obj(_Robot->getRobotStruct(),_Humans[j]->getRobotStruct()->o[i]);
                p3d_col_deactivate_rob_obj(_Humans[j]->getRobotStruct(),_Humans[j]->getRobotStruct()->o[i]);
                //                cout << "Deactivate " << _Robot->getName() << " to " << _Humans[j]->getRobotStruct()->o[i]->name << endl;
            }
            else
            {
                //                p3d_col_activate_rob_obj(_Robot->getRobotStruct(),_Humans[j]->getRobotStruct()->o[i]);
