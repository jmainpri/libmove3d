...
shared_ptr<Configuration> RRT::diffuseOneConf(
	...
	return (path->configAtParam(min(path->length(),this->step())));
}
...
int RRT::expandOneStep(Node* fromComp, Node* toComp) {
	...
	if (ENV.getBool(Env::isManhattan) && !(this->manhattanSamplePassive())) {
		...
		int nbCreatedNodes = this->ExpandProcess(expansionNode,
	...
	// Standard expansion case
		...
		// expansion
		return (this->ExpandProcess(expansionNode, directionConfig,
				directionNode, ENV.getExpansionMethod()));
	}
}

bool RRT::nextStep(LocalPath& path, Node* directionNode, double& pathDelta,
	...
	if (method == Env::Connect) {
		...
		pathDelta = path.length() == 0. ? 1.
				: MIN(1., this->step() / path.length());
...
void RRT::expansionFailed(Node* node) {
	...
	if (ENV.getBool(Env::discardNodes) && (node->getNodeStruct()
			!= _Graph->getGraphStruct()->search_start)
			&& (node->getNodeStruct() != _Graph->getGraphStruct()->search_goal)
			&& (node->getNodeStruct()->n_fail_extend > ENV.getInt(
					Env::MaxExpandNodeFail))) {
...
int RRT::ExpandProcess(Node* expansionNode,
	...
	while (firstIteration || (method == Env::nExtend && !failed
		...
		if (!failed) {
			extensionNode = this->connectNode(fromNode, *extensionLocalpath,
...
int RRT::passiveExpandProcess(Node* expansionNode, int NbActiveNodesCreated,
	...
	while (reiterate) {
		...
		p3d_copy_config_into(
				_Robot->getRobotStruct(),
				_Robot->getRobotStruct()->currect_q_inv,
				&q);
		...
		if (getCollidingPassiveJntList(_Robot->getRobotStruct(),
			...
			if (selectNewJntInList(_Robot->getRobotStruct(), joints, oldJoints,
				...
				for (int i(0); !expansionSucceeded; i++) {
					...
					int nbCreatedNodes = this->ExpandProcess(lastCreatedNode,
...
bool RRT::expandControl(LocalPath& path, double positionAlongDirection,
		Node* compNode) {
	if (path.length() <= this->step() && positionAlongDirection >= 1.) {
		if (compNode->getCompcoStruct()->nbRefinNodes * 2
				> compNode->getCompcoStruct()->nnode)
			return (false);
		else {
...
uint RRT::run() {
	...
	while (!this->checkStopConditions()) {
		...
		if (!(ENV.getBool(Env::biDir) && ENV.getBool(Env::expandBalanced)
			...
			NbCurCreatedNodes = (ENV.getBool(Env::biDir) ? this->expandOneStep(
					fromNode, toNode) : this->expandOneStep(_Start, _Goal));
