...
int urdf_p3d_converter(URDFModel* model, char* modelName, double scale)
{
    ...
    if(root_link->visual)
    {
        ...
        {
            double color_vect[4]={root_link->visual->material->color.r,
                                  root_link->visual->material->color.g,
                                  root_link->visual->material->color.b,
                                  root_link->visual->material->color.a};

            p3d_poly_set_color(p3d_poly_get_poly_by_name((char*)root_link->name.c_str()), Any, color_vect );
        }
//        else
//        {
//            cout << "no color for mesh " << endl;
//        }
    ...
    if( root_link->visual != NULL )
    {
        origin = root_link->visual->origin;
    }

    parcoursArbre( root_link, num_prev_jnt, &num_last_jnt, origin, scale );
...
void parcoursArbre(boost::shared_ptr<const Link> link_parent, int num_prev_jnt, int * num_last_jnt, Pose pos_abs_jnt_parent, double scale)
{
    ...
    for (std::vector< boost::shared_ptr<Link> >::const_iterator child = link_parent->child_links.begin();
         child != link_parent->child_links.end();
         child++)
    {
        if (*child)
        {
            ...
            if((*child)->visual)
            {
                ...
                {
                    double color_vect[4]={(*child)->visual->material->color.r,
                                          (*child)->visual->material->color.g,
                                          (*child)->visual->material->color.b,
                                          (*child)->visual->material->color.a};

                    // cout << "set poly " << (*child)->name << " at color : " ;
                    // cout << color_vect[0] << " " << color_vect[1] << " , " << color_vect[2] << " , " << color_vect[3] << endl;
    ...
    for(int i=0; i<node->mNumChildren; i++)
    {
        ...
        for(unsigned int j=0; j< mChildren->mNumMeshes; j++)
        {
            ...
            for(unsigned int k=0; k<input_mesh->mNumVertices; k++){
                ...
                //cout << "add vert (" << p.x  << " , " << p.y << " , " << p.z << ")" << endl;
            }
            // faces
            for(unsigned int k=0; k<input_mesh->mNumFaces; k++)
            {
                //cout << "parcourt des faces" << k << endl;
                ...
                if( face.mNumIndices == 3 ) {
                    ...
                    //cout << "add face (" << indices[0]  << " , " << indices[1] << " , " << indices[2] << ")" << endl;
                }
            }
        }
...
void loadMesh(std::string const& filename)
{
    ...
    if(!scene){
        cout << "failed to load " << filename << " in move3d loadMesh" << endl;
        return;
    }
    if(!scene->mRootNode){
        cout << "resource " << filename << " has no data" << endl;
        return;
    }
    if(!scene->HasMeshes()){
        cout << "no meshes found in file " << filename << endl;
    }
