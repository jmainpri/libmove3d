...
int p3d_desactivate_col_check_automatic(void) {
  int i,j, father_body_found,body_index,nof_obj_for_rob;
  int *hierarchical_father;
  float epsilon = 0.1;
  p3d_jnt * ajntPt;
  ...
  nof_obj_for_rob = ROB_AUTOCOL->nof_obj_for_robots[ROB_AUTOCOL->cur_rob_id];
  /*** REGLE N*1 : DESACTIVER LES BODIES VIDES (SANS POLYHEDRE) 
       AVEC TOUS LES AUTRES BODIES. ***/
  ...
  for (i=0; i<nof_obj_for_rob; i++) {
	...
	if (ajntPt->prev_jnt!=NULL) {
	  ...
	  if (ajntPt->o != NULL) 
	    if (ajntPt->o->np > 0) father_body_found = 1;
  ...
  for (i=0; i<nof_obj_for_rob; i++) {
    /*** EXISTE T IL UN PERE IDENTIQUE POUR DEUX BODIES NON VIDES POSSEDANT EUX MEME UN PERE ? ***/
    if (hierarchical_father[i] > -1 && XYZ_ENV->cur_robot->o[i]->np >0) {
