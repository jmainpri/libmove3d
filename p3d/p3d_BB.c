...
/********************************************/
/* Fonction ne faisant rien (BB désactivé)  */
/* In : le polyhedre                        */
/* Out :                                    */
/********************************************/
...
{ 
  ...
  u[0]=p->box.x1;  v[0]=p->box.x2; 
  u[1]=p->box.y1;  v[1]=p->box.y2; 
  u[2]=p->box.z1;  v[2]=p->box.z2; 
...
{int np,i;
 ...
 p3d_BB *BB;
  ...
  BB->xmin=P3D_HUGE; BB->xmax=-P3D_HUGE;
  BB->ymin=P3D_HUGE; BB->ymax=-P3D_HUGE;
  BB->zmin=P3D_HUGE; BB->zmax=-P3D_HUGE;

  for(i=0;i<np;i++){ 
    if (obj->pol[i]->TYPE != P3D_GRAPHIC){  // Modification Fabien
      p3d_BB_get_BB_poly(obj->pol[i],&x1,&x2,&y1,&y2,&z1,&z2);
      if(x1<BB->xmin){BB->xmin=x1;}
      if(x2>BB->xmax){BB->xmax=x2;}
      if(y1<BB->ymin){BB->ymin=y1;}
      if(y2>BB->ymax){BB->ymax=y2;}
      if(z1<BB->zmin){BB->zmin=z1;}
      if(z2>BB->zmax){BB->zmax=z2;}
    }
  }
...
{ int i,j;
 ...
 PrintInfo(("Initialisation des BB dans le repere propre de l objet, terminé.\n"));
}
...
{ p3d_BB *BB,*BB0;
 p3d_matrix4 inv_pos0, tmp;
  ...
  u[0]=BB0->xmin;  v[0]=BB0->xmax; 
  u[1]=BB0->ymin;  v[1]=BB0->ymax; 
  u[2]=BB0->zmin;  v[2]=BB0->zmax;
  if (obj->jnt != NULL) {
...
{ int no,i;
  ...
  for(i=0;i<no;i++)
  { BBo = &(rob->o[i]->BB);
    if(BBo->xmin<BB->xmin){BB->xmin=BBo->xmin;}
    if(BBo->xmax>BB->xmax){BB->xmax=BBo->xmax;}
    if(BBo->ymin<BB->ymin){BB->ymin=BBo->ymin;}
    if(BBo->ymax>BB->ymax){BB->ymax=BBo->ymax;}
    if(BBo->zmin<BB->zmin){BB->zmin=BBo->zmin;}
    if(BBo->zmax>BB->zmax){BB->zmax=BBo->zmax;}
  } 
...
{ double o1min[3],o1max[3],o2min[3],o2max[3];
  ...
  o1min[0]=obj1->BB.xmin; o1max[0]=obj1->BB.xmax;
  o1min[1]=obj1->BB.ymin; o1max[1]=obj1->BB.ymax;
  o1min[2]=obj1->BB.zmin; o1max[2]=obj1->BB.zmax;

  o2min[0]=obj2->BB.xmin; o2max[0]=obj2->BB.xmax;
  o2min[1]=obj2->BB.ymin; o2max[1]=obj2->BB.ymax;
  o2min[2]=obj2->BB.zmin; o2max[2]=obj2->BB.zmax;
...
{ double rmin[3],rmax[3],omin[3],omax[3];
   ...
   rmin[0]=rob->BB.xmin; rmax[0]=rob->BB.xmax; 
   rmin[1]=rob->BB.ymin; rmax[1]=rob->BB.ymax; 
   rmin[2]=rob->BB.zmin; rmax[2]=rob->BB.zmax; 

   omin[0]=obj->BB.xmin; omax[0]=obj->BB.xmax; 
   omin[1]=obj->BB.ymin; omax[1]=obj->BB.ymax; 
   omin[2]=obj->BB.zmin; omax[2]=obj->BB.zmax; 
...
{double r1min[3],r1max[3],r2min[3],r2max[3];
   ...
   r1min[0]=rob1->BB.xmin; r1max[0]=rob1->BB.xmax; 
   r1min[1]=rob1->BB.ymin; r1max[1]=rob1->BB.ymax; 
   r1min[2]=rob1->BB.zmin; r1max[2]=rob1->BB.zmax; 

   r2min[0]=rob2->BB.xmin; r2max[0]=rob2->BB.xmax; 
   r2min[1]=rob2->BB.ymin; r2max[1]=rob2->BB.ymax; 
   r2min[2]=rob2->BB.zmin; r2max[2]=rob2->BB.zmax; 
...
/*  PrintInfo(("axe 3 : [%f %f] [%f %f]\n",o1min[2],o1max[2],o2min[2],o2max[2]));  */
 ...
 if((o1max[0]>=o2min[0]&&o1min[0]<=o2max[0]) || (o1min[0]<=o2max[0]&&o2min[0]<=o1max[0])){cx=1;}
 if((o1max[1]>=o2min[1]&&o1min[1]<=o2max[1]) || (o1min[1]<=o2max[1]&&o2min[1]<=o1max[1])){cy=1;} 
 if((o1max[2]>=o2min[2]&&o1min[2]<=o2max[2]) || (o1min[2]<=o2max[2]&&o2min[2]<=o1max[2])){cz=1;}
 /* PrintInfo(("cx : %d, cy : %d, cz : %d\n",cx,cy,cz)); */

 /* les boites se chevauchent */
 if(cx && cy && cz){
   d = 0.;
   if(o1max[0]>=o2min[0]&&o1min[0]<=o2max[0]){*dist_ut = SQR(o2min[0]-o1max[0]);}
   else{*dist_ut = SQR(o1min[0]-o2max[0]);}
   if(o1max[1]>=o2min[1]&&o1min[1]<=o2max[1]){*dist_ut = *dist_ut + SQR(o2min[1]-o1max[1]);}
   else{*dist_ut = *dist_ut + SQR(o1min[1]-o2max[1]);}
   if(o1max[2]>=o2min[2]&&o1min[2]<=o2max[2]){*dist_ut = *dist_ut + SQR(o2min[2]-o1max[2]);}
   else{*dist_ut = *dist_ut + SQR(o1min[2]-o2max[2]);}
   *dist_ut = sqrt(*dist_ut);
 }
 /* deux faces se chevauchent en projection */
 else if(cx && cy && !cz){
   if(o1max[2]<=o2min[2]&&o1min[2]<=o2max[2]){d = fabs(o2min[2]-o1max[2]);}
   else{d = fabs(o1min[2]-o2max[2]);}
   if(o1max[0]>=o2min[0]&&o1min[0]<=o2max[0]){*dist_ut = SQR(o2min[0]-o1max[0]);}
   else{*dist_ut = SQR(o1min[0]-o2max[0]);}
   if(o1max[1]>=o2min[1]&&o1min[1]<=o2max[1]){*dist_ut = *dist_ut + SQR(o2min[1]-o1max[1]);}
   else{*dist_ut = *dist_ut + SQR(o1min[1]-o2max[1]);}
   *dist_ut = sqrt(*dist_ut);
   ...
   if(o1max[1]<=o2min[1]&&o1min[1]<=o2max[1]){d = fabs(o2min[1]-o1max[1]);}
   else{d = fabs(o1min[1]-o2max[1]);}
   if(o1max[0]>=o2min[0]&&o1min[0]<=o2max[0]){*dist_ut = SQR(o2min[0]-o1max[0]);}
   else{*dist_ut = SQR(o1min[0]-o2max[0]);}
   if(o1max[2]>=o2min[2]&&o1min[2]<=o2max[2]){*dist_ut = *dist_ut + SQR(o2min[2]-o1max[2]);}
   else{*dist_ut = *dist_ut + SQR(o1min[2]-o2max[2]);}
   *dist_ut = sqrt(*dist_ut);
   ...
   if(o1max[0]<=o2min[0]&&o1min[0]<=o2max[0]){d = fabs(o2min[0]-o1max[0]);}
   else{d = fabs(o1min[0]-o2max[0]);}
   if(o1max[1]>=o2min[1]&&o1min[1]<=o2max[1]){*dist_ut = SQR(o2min[1]-o1max[1]);}
   else{*dist_ut = SQR(o1min[1]-o2max[1]);}
   if(o1max[2]>=o2min[2]&&o1min[2]<=o2max[2]){*dist_ut = *dist_ut + SQR(o2min[2]-o1max[2]);}
   else{*dist_ut = *dist_ut + SQR(o1min[2]-o2max[2]);}
   *dist_ut = sqrt(*dist_ut);
 }
 /* deux aretes se chevauchent en projection */
   ...
   if(o1max[1]<=o2min[1]&&o1min[1]<=o2max[1]){d = SQR(o2min[1]-o1max[1]);}
   else{d = SQR(o1min[1]-o2max[1]);}
   if(o1max[2]<=o2min[2]&&o1min[2]<=o2max[2]){d = d + SQR(o1max[2]-o2min[2]);}
   else{d = d + SQR(o1min[2]-o2max[2]);}
   d = sqrt(d);
   if(o1max[0]<=o2min[0]&&o1min[0]<=o2max[0]){*dist_ut = fabs(o2min[0]-o1max[0]);}
   else{*dist_ut = fabs(o1min[0]-o2max[0]);}
 }
 else if(cy && !cx && !cz){
   if(o1max[0]<=o2min[0]&&o1min[0]<=o2max[0]){d = SQR(o1max[0]-o2min[0]);}
   else{d = SQR(o2max[0]-o1min[0]);}
   if(o1max[2]<=o2min[2]&&o1min[2]<=o2max[2]){d = d + SQR(o1max[2]-o2min[2]);}
   else{d = d + SQR(o2max[2]-o1min[2]);}
   d = sqrt(d);
   if(o1max[1]<=o2min[1]&&o1min[1]<=o2max[1]){*dist_ut = fabs(o2min[1]-o1max[1]);}
   else{*dist_ut = fabs(o1min[1]-o2max[1]);}			  
 }
 else if(cz && !cx && !cy){
   if(o1max[0]<=o2min[0]&&o1min[0]<=o2max[0]){d = SQR(o1max[0]-o2min[0]);}
   else{d = SQR(o2max[0]-o1min[0]);}
   if(o1max[1]<=o2min[1]&&o1min[1]<=o2max[1]){d = d + SQR(o1max[1]-o2min[1]);}
   else{d = d + SQR(o2max[1]-o1min[1]);}
   d = sqrt(d);
   if(o1max[2]<=o2min[2]&&o1min[2]<=o2max[2]){*dist_ut = fabs(o2min[2]-o1max[2]);}
   else{*dist_ut = fabs(o1min[2]-o2max[2]);}
 }
 /* aucun chevauchement meme en projection */
 else{if(o1max[0]<=o2min[0]&&o1min[0]<=o2max[0]){d = SQR(o1max[0]-o2min[0]);}
      else{d = SQR(o2max[0]-o1min[0]);}
      if(o1max[1]<=o2min[1]&&o1min[1]<=o2max[1]){d = d + SQR(o1max[1]-o2min[1]);}
      else{d = d + SQR(o2max[1]-o1min[1]);}
      if(o1max[2]<=o2min[2]&&o1min[2]<=o2max[2]){d = d + SQR(o1max[2]-o2min[2]);}
...
{ int i,j,k,np; 
  ...
  BB0->xmin=P3D_HUGE; BB0->xmax=-P3D_HUGE;
  BB0->ymin=P3D_HUGE; BB0->ymax=-P3D_HUGE;
  BB0->zmin=P3D_HUGE; BB0->zmax=-P3D_HUGE;
  ...
  for(i=0;i<np;i++) {
    if (obj->pol[i]->TYPE != P3D_GRAPHIC) {  // Modification Fabien
      ...
      u[0]=obj->pol[i]->box.x1;  v[0]=obj->pol[i]->box.x2; 
      u[1]=obj->pol[i]->box.y1;  v[1]=obj->pol[i]->box.y2; 
      u[2]=obj->pol[i]->box.z1;  v[2]=obj->pol[i]->box.z2; 
      for(j=0;j<3;j++)  
      for(k=0;k<3;k++) 
        { if (mat[j][k]>0) 
            { w[j][k]=u[k]; 
      ...
      if(x1<BB0->xmin){BB0->xmin=x1;}
      if(x2>BB0->xmax){BB0->xmax=x2;}
      if(y1<BB0->ymin){BB0->ymin=y1;}
      if(y2>BB0->ymax){BB0->ymax=y2;}
      if(z1<BB0->zmin){BB0->zmin=z1;}
      if(z2>BB0->zmax){BB0->zmax=z2;}
    } 
  }
