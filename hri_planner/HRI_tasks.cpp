...
//                   p3d_destroy_config( manipulation->robot(), openConf );
                  ...
                  if ( status==MANIPULATION_TASK_OK )
                  {
                     ...
                     g3d_is_object_visible_from_viewpoint ( primary_human_MM->perspective->camjoint->abs_pos, primary_human_MM->perspective->fov, object, &visibility, 1 );
                     printf ( " visibility=%lf\n",visibility );

                     for ( int i1=0;i1<candidate_points_to_give.no_points;i1++ )
                     {
                        printf ( "checking for place %d and grasp id= %d\n",i1, igrasp->ID );
                        //          if(i1==0)
                        //     goal_pos.x=candidate_points_to_put.point[i1].x +0.5;
