...
// right arm 1,2,3
//  left arm 4,5,6
void hri_set_human_config_from_kinect( HRI_AGENTS * agents , configPt q , HumanKinect& humKin )
{
  p3d_rob * robot = NULL;
  p3d_jnt* joint = NULL;
  int index_dof;
  ...
  // Set pelvis
  joint = p3d_get_robot_jnt_by_name(robot, "Pelvis");
  index_dof = joint->index_dof;

  q[index_dof] = humKin.TORSO[0];
  q[index_dof] = humKin.TORSO[1];
  q[index_dof] = humKin.TORSO[2];

  // Right arm
  joint = p3d_get_robot_jnt_by_name(robot, "rShoulderX");
  
  humKin.SHOULDER_RIGHT[0];
  humKin.SHOULDER_RIGHT[1];
  humKin.SHOULDER_RIGHT[0];
