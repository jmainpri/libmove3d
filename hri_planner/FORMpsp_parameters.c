...
{
  PSP_PARAMETERS_FORM = fl_bgn_form(FL_UP_BOX,370.0,400.0);
...
{  
  ...
  obj = fl_add_frame(FL_ENGRAVED_FRAME,30,210,280,85,""); 
...
{  
  ...
  obj2 = fl_add_frame(FL_ENGRAVED_FRAME,30,315,280,70,""); 
  obj2 = fl_add_box(FL_FLAT_BOX,45,310,60,10,"View Field"); 
  
  double distoriY=330.0;
  BR_PSP_VF_HANGLE = fl_add_valslider(FL_HOR_SLIDER,40.0,distoriY,245.0,10.0,"Horizontal Angle");
  ...
  BR_PSP_VF_VANGLE = fl_add_valslider(FL_HOR_SLIDER,40.0,distoriY+25,245.0,10.0,"Vertical Angle");
  ...
  if (arg==0)
    {
      ...
      p3d_set_rob_cam_parameters(PSP_ROBOT,-0.20,.0,-0.10,3.0,7.0,0.7,1.05,9,0,.5,.0);
  ...
  else
    {
     ...
     // psr_get_human_pointing_from_joint_number(tHuman,tRobot,56);
      ...
      psp_srch_for_target_obj(tRobot,20,0,BTSET);
...
static void g3d_create_btns(void)
{
  ...
  BTN_PSP_GO_FOR_OBJECT= fl_add_button(FL_PUSH_BUTTON,300,60,50,30,"GO");
...
{  
  ...
  fl_set_slider_step(BR_CAM_X,0.01); 
  ...
  fl_set_slider_step(BR_CAM_Y,0.01); 
  ...
  fl_set_slider_step(BR_CAM_Z,0.01); 
...
static void CB_win_mode(FL_OBJECT *ob, long arg)
{
  ...
  /* There are couple of compiling errors in the code below. Commented for now */
  ...
  /*   g3d_set_win_draw_mode(persp_win,arg); */
