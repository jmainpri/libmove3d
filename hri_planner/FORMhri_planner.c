...
static void CB_jointsel_obj(FL_OBJECT *obj, long arg);
...
static void g3d_create_GIK_group(void)
{
  ...
  GIK_STEP_OBJ = fl_add_valslider(FL_HOR_SLIDER,framex+150,framey+39,100,15,"");
	...
	GIK_JOINTSEL_OBJ = fl_add_button(FL_NORMAL_BUTTON,framex+250,framey+30,80,25,"Joint Selection");
  ...
  fl_set_slider_value(GIK_STEP_OBJ,GIK_STEP);
	...
	fl_set_object_callback(GIK_JOINTSEL_OBJ,CB_jointsel_obj,1);
...
static void CB_run_GIKstep_obj(FL_OBJECT *ob, long arg)
{
  ...
  hri_gik_computestepWoP(BTSET->robot, HRI_GIK, GIK_STEP, GIK_DIRECT, GIK_FORCE);
...
static void CB_run_GIK_obj(FL_OBJECT *ob, long arg)
{
	...
	p3d_vector3 Tcoord[3],Torient[3];
  p3d_rob * visball = NULL;
  int i;
  configPt q;
  ...
  for(i=0; i<env->nr; i++){
	...
	p3d_mat4ExtractPosReverseOrder(visball->joints[1]->abs_pos, &Tcoord[0][0], &Tcoord[0][1], &Tcoord[0][2], &Torient[0][0], &Torient[0][1], &Torient[0][2]);
	p3d_mat4ExtractPosReverseOrder(visball->joints[1]->abs_pos, &Tcoord[1][0], &Tcoord[1][1], &Tcoord[1][2], &Torient[0][0], &Torient[0][1], &Torient[0][2]);
	p3d_mat4ExtractPosReverseOrder(visball->joints[1]->abs_pos, &Tcoord[2][0], &Tcoord[2][1], &Tcoord[2][2], &Torient[0][0], &Torient[0][1], &Torient[0][2]);

  hri_gik_compute(robotPt, HRI_GIK, GIK_STEP, 0.01, GIK_DIRECT, GIK_FORCE, Tcoord, NULL, &q, NULL);
