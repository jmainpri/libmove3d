...
p3d_convert_ptpTraj_to_smoothedTraj(&gain, &ntest, trajSmPTPPt, trajSmPt);
  robotPt->tcur = trajSmPt;
	...
	if(param_write_file == true) {
	  p3d_softMotion_export_traj(robotPt, trajSmPTPPt, 0, (char*)"arm.traj", ENV.getBool(Env::plotSoftMotionCurve),  lp, positions, smTraj) ;
	  //p3d_softMotion_write_curve_for_bltplot(robotPt, trajSmPTPPt, (char*)"RefSM.dat",ENV.getBool(Env::plotSoftMotionCurve) , lp, positions, smTraj) ;
	  smTraj.clear();
	  p3d_softMotion_export_traj(robotPt, trajSmPt, 1, (char*)"arm2.traj", ENV.getBool(Env::plotSoftMotionCurve),  lp, positions, smTraj) ;
	  //p3d_softMotion_write_single_curve_for_bltplot(robotPt, trajSmPt, (char*)"RefSM2.dat",ENV.getBool(Env::plotSoftMotionCurve) , lp, positions, smTraj) ;
	}
	if (fct_draw){(*fct_draw)();}
