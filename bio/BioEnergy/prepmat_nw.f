...
!     new arguments for MOVE3D version 
                           mat,imatmax,index1,index2)
!
!     Scalar Arguments
!
      character        fmtx*(*)
      integer          natblocs, natom, nb, maxsize
!
!     Array Arguments
!
...
!      --------------------------------------------------------
!         no input file for MOVE3D version
!         mat (imat),index1(imat),index2(imat) 
!         instead of i,j,toto
!         do loop instead of read (50,*,end=99), 
!         99 continue and goto 50   
!      --------------------------------------------------------
...
!         if (i.gt.3*natom.or.j.gt.3*natom) then
         if (index1(imat).gt.3*natom.or.index2(imat).gt.3*natom) then
...
!
!         if (i.eq.j) then
         if (index1(imat).eq.index2(imat)) then
!         if (.not.qtrace(i)) then
         if (.not.qtrace(index1(imat))) then
!             trace=trace+toto
              trace=trace+mat(imat)
!             qtrace(i)=.true. 
             qtrace(index1(imat))=.true. 
         endif
         endif
...
!         if (j.gt.imax) imax=j
         ...
         if (index1(imat).gt.imax) imax=index1(imat)
         if (index2(imat).gt.imax) imax=index2(imat)
...
!	    qinit=.false.
!         endif
         IF (index1(imat).le.NI2) then
            iii=index1(imat)-NTOTN1
            A(iii,index2(imat))= mat(imat)
	    qinit=.false.
         endif
!
...
!     
!     -------------------------------------------------------- 
!      enddo for loop(imat) in MOVE3D version
!     --------------------------------------------------------    
      enddo
...
!      write(6,'(A,2I6,F15.4)') ' Last element read: ',i,j,toto
      write(6,'(A,2I6,F15.4)') ' Last element read: ',&
      index1(imatmax),index2(imatmax),mat(imatmax)

!            iii=i-NTOTN1
!            A(iii,j)=toto
            iii=index1(imatmax)-NTOTN1
            A(iii,index2(imatmax))=mat(imatmax)
            NJ1=NI1
            NJ2=NI2
	    ii=0
               do i=1,TABB(nn)
