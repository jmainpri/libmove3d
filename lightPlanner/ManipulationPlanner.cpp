...
#include <p3d_constraints_proto.h>
...
ManipulationPlanner::ManipulationPlanner(p3d_rob *robotPt) { //
  ...
  for (int i = 0; i < _robot->mlp->nblpGp; i++) {
...
int ManipulationPlanner::addConfigTraj(configPt config) {
  ...
  new_conf = p3d_alloc_config(_robot);
  p3d_copy_config_into(_robot, config, &new_conf);
...
int ManipulationPlanner::clearConfigTraj() {
  ...
  for (i = 0; i < _configTraj.size(); ++i)  {
    p3d_destroy_config(_robot, _configTraj[i]);
  }
  _configTraj.clear();
...
int ManipulationPlanner::copyConfigTrajToFORM() {
  ...
  for (i = 0; i < _configTraj.size(); ++i) {
    ...
    p3d_set_new_robot_config(name, _configTraj[i], _robot->ikSol, _robot->confcur);
    _robot->confcur = _robot->conf[0];
#ifdef XFORMS
...
//   configPt q = NULL;
// 
//   q = p3d_alloc_config(_robot);
//   p3d_get_robot_config_into(_robot, &q);
// 
//   ////////////////////////q1////////////////////////////
//   armJoint = p3d_get_robot_jnt_by_name(_robot, (char*)GP_ARMJOINT1);
//   if (armJoint == NULL) {
...
// 
//   ////////////////////////q2////////////////////////////
//   armJoint = p3d_get_robot_jnt_by_name(_robot, (char*)GP_ARMJOINT2);
//   if (armJoint == NULL) {
//     p3d_destroy_config(_robot, q);
//     return 1;
//   }
...
// 
//   ////////////////////////q3////////////////////////////
//   armJoint = p3d_get_robot_jnt_by_name(_robot, (char*)GP_ARMJOINT3);
//   if (armJoint == NULL) {
//     p3d_destroy_config(_robot, q);
//     return 1;
//   }
...
// 
//   ////////////////////////q4////////////////////////////
//   armJoint = p3d_get_robot_jnt_by_name(_robot, (char*)GP_ARMJOINT4);
//   if (armJoint == NULL) {
//     p3d_destroy_config(_robot, q);
//     return 1;
//   }
...
// 
//   ////////////////////////q5////////////////////////////
//   armJoint = p3d_get_robot_jnt_by_name(_robot, (char*)GP_ARMJOINT5);
//   if (armJoint == NULL) {
//     p3d_destroy_config(_robot, q);
//     return 1;
//   }
...
// 
//   ////////////////////////q6////////////////////////////
//   armJoint = p3d_get_robot_jnt_by_name(_robot, (char*)GP_WRISTJOINT);
//   if (armJoint == NULL) {
//     p3d_destroy_config(_robot, q);
//     return 1;
//   }
...
//   result = p3d_set_and_update_this_robot_conf(_robot, q);
//   p3d_get_robot_config_into(_robot, &_robot->ROBOT_POS);
// 
//   p3d_destroy_config(_robot, q);
// 
//   return !result;
...
//   qcur = p3d_alloc_config(_robot);
//   p3d_get_robot_config_into(_robot, &qcur);
// 
//   result = p3d_set_virtual_object_pose2(_robot, x, y, z, rx, ry, rz);
//   deactivateCcCntrts(_robot, -1);
//   if (result == TRUE) {
//     p3d_destroy_config(_robot, qcur);
//     return 0;
//   } else {
// //     printf("%s: %d: ManipulationPlanner::setArmX(): the input pose is not reachable (no IK solution).\n",__FILE__,__LINE__);
//     p3d_set_and_update_this_robot_conf(_robot, qcur); //restore the initial configuration
//     p3d_destroy_config(_robot, qcur);
//     return 1;
//   }
// }
...
// 
//     if (result == 0 && !p3d_col_test_robot(_robot, 0)) {
//       p3d_destroy_config(_robot, qcur);
//       return 0;
//     }
//   }
...
//     return 1;
//   }
//   return p3d_get_virtual_object_pose2(_robot, x, y, z, rx, ry, rz);
// }
// 
//Arms Gestion
...
void ManipulationPlanner::setArmCartesian(int armId, bool cartesian) {
  ...
  if (cartesian) {
    p3d_activateCntrt(_robot, armData.getCcCntrt());
    ...
    p3d_desactivateCntrt(_robot, armData.getCcCntrt());
...
bool ManipulationPlanner::getArmCartesian(int armId) {
  return (*_robot->armManipulationData)[armId].getCartesian();
}

//Hands Gestion

void ManipulationPlanner::fixAllHands(configPt q){
  p3d_set_and_update_this_robot_conf(_robot, q);
  for(uint i = 0; i < (*_robot->armManipulationData).size(); i++){
    (*_robot->armManipulationData)[i].fixHand(_robot);
  }
...
int ManipulationPlanner::getOptimizeSteps(void){
  return _optimizeSteps;
}

double ManipulationPlanner::getOptimizeTime(void){
...
int ManipulationPlanner::printConstraintInfo() {
  ...
  printf("robot \"%s\" has %d constraints: \n", _robot->name, _robot->cntrt_manager->ncntrts);
  for (i = 0; i < _robot->cntrt_manager->ncntrts; i++) {
    printf("%s, active= %d\n", _robot->cntrt_manager->cntrts[i]->namecntrt, _robot->cntrt_manager->cntrts[i]->active);
  }
  return 0;
}
...
//   p3d_set_virtual_object_pose(_robot, T1);
...
//         gpOpen_hand(_robot, _handProp.at(armId));
//         qi = p3d_get_robot_config(_robot);
//         addConfigTraj(qi);
// 
//         qgrasp = p3d_alloc_config(_robot);
//         qpregrasp = p3d_alloc_config(_robot);
...
//         //p3d_activateCntrt(_robot, _robot->fkCntrts[0]);
// 
//         p3d_set_and_update_this_robot_conf(_robot, qpregrasp);
//         gpOpen_hand(_robot, _handProp.at(armId));
// 
//         qint = p3d_get_robot_config(_robot);
//         addConfigTraj(qint);
// 
//         p3d_set_and_update_this_robot_conf(_robot, qgrasp);
//         gpOpen_hand(_robot, _handProp.at(armId));
// 
//         qf = p3d_get_robot_config(_robot);
...
// 
// #ifdef FK_CNTRT
//           if (_cartesian == true && _robot->nbFkCntrts != 0) {
//             p3d_desactivateCntrt(_robot, _robot->fkCntrts[0]);
//           }
// #endif
// 
...
//       }
// 
//       GP_ConcateneAllTrajectories(_robot);
// 
//       _robot->tcur = _robot->t[0];
//       p3d_copy_config_into(_robot, qi, &qStart);
// 
...
//       printf("il y a %zd configurations\n", _configTraj.size());
// 
//       qi = p3d_copy_config(_robot, qStart);
...
// 
// 
//       qf = p3d_alloc_config(_robot);
//       p3d_copy_config_into(_robot, qGoal, &qf);
//       p3d_update_virtual_object_config_for_arm_ik_constraint(_robot, armId, qf);
//       gpOpen_hand(_robot, _handProp.at(armId));
//       p3d_set_and_update_this_robot_conf(_robot, qf);
//       p3d_destroy_config(_robot, qf);
//       qf = p3d_get_robot_config(_robot);
//       addConfigTraj(qf);
// 
//       if (p3d_equal_config(_robot, qStart, qGoal)) {
...
// 
//   if (p3d_set_object_to_carry(_robot, objectName) != 0) {
//     return 1;
//   }
//   p3d_grab_object2(_robot, 0);
//   FORMrobot_update(p3d_get_desc_curnum(P3D_ROBOT));
//   g3d_draw_allwin_active();
// 
//   return 0;
...
//   configPt qi = NULL;
//   if (p3d_release_object(_robot) == 1) {
//     return 1;
//   }
//   deactivateCcCntrts(_robot, -1);
//   qi = p3d_alloc_config(_robot);
//   p3d_copy_config_into(_robot, _robot->ROBOT_POS, &qi);
//   /* Uptdate the Virual object for inverse kinematics */
//   p3d_update_virtual_object_config(_robot, 0, qi);
//   //p3d_update_virtual_object_config_for_pa10_6_arm_ik_constraint(_robot, qi);
//   p3d_set_and_update_this_robot_conf(_robot, qi);
//   p3d_destroy_config(_robot, qi);
//   qi = p3d_get_robot_config(_robot);
//   p3d_copy_config_into(_robot, qi, &_robot->ROBOT_POS);
...
//     qf = p3d_alloc_config(_robot);
//     p3d_multiLocalPath_disable_all_groupToPlan(_robot);
//     p3d_multiLocalPath_set_groupToPlan(_robot, _ObjectMLP, 1);
//     //p3d_multiLocalPath_set_groupToPlan_by_name(_robot, (char*)"jido-ob_lin", 1) ;
//     p3d_copy_config_into(_robot, _robot->ROBOT_POS, &qi);
//     p3d_copy_config_into(_robot, _robot->ROBOT_GOTO, &qf);
//     p3d_update_virtual_object_config_for_arm_ik_constraint(_robot, 0, qi);
//     p3d_update_virtual_object_config_for_arm_ik_constraint(_robot, 0, qf);
//     p3d_copy_config_into(_robot, qi, &_robot->ROBOT_POS);
//     p3d_copy_config_into(_robot, qf, &_robot->ROBOT_GOTO);
//     p3d_destroy_config(_robot, qi);
//     p3d_destroy_config(_robot, qf);
//     if (_robot->nbCcCntrts != 0) {
//       for (int w = 0; w < _robot->nbCcCntrts; w++) {
//         p3d_activateCntrt(_robot, _robot->ccCntrts[w]);
//       }
//     }
//   }
// 
...
//   if (_cameraJnt != NULL) {
//     qbase = p3d_alloc_config(_robot);
//     p3d_get_robot_config_into(_robot, &qbase);
//     gpGrasp_visibility_filter(_robot, _object, _cameraJnt, _cameraFOV, _cameraImageWidth, _cameraImageHeight, _graspList.at(armId), GP_PA10, qbase, _handProp.at(armId));
//     p3d_destroy_config(_robot, qbase);
//   }
// 
// 
//   allowWindowEvents();
...
// 
//     result = p3d_set_virtual_object_pose2(_robot, x, y, z, 0, 0, theta);
// 
//     if (result == 0) {
//       unreachable = FALSE;
//       gpOpen_hand(_robot, hand);
// 
//       if (p3d_col_test()) {
//         continue;
//       } else {
//         gpClose_hand(_robot, hand);
// 
// //           if(!p3d_col_test_robot_obj(robotPt, OBJECT)) {
//         if (!p3d_col_test_robot_other(_robot, _object, 0)) {
//           continue;
//         }
// 
//         gpOpen_hand(_robot, hand);
...
//   }
// 
//   result = gpFind_grasp_and_pregrasp_from_base_configuration(_robot, _object, untestedGrasps, GP_LWR, qcur, _grasp.at(armId), _handProp.at(armId), _liftUpDistance, qpregrasp, qgrasp);
// 
...
//     p3d_set_and_update_this_robot_conf(_robot, qcur);
//     p3d_destroy_config(_robot, qcur);
//     p3d_destroy_config(_robot, qpregrasp);
//     p3d_destroy_config(_robot, qgrasp);
//     XYZ_ENV->cur_robot = cur_robot;
...
// 
//   p3d_get_robot_config_into(_robot, &_robot->ROBOT_GOTO);
// 
//   p3d_set_and_update_this_robot_conf(_robot, qcur);
//   p3d_get_robot_config_into(_robot, &_robot->ROBOT_POS);
// 
//   p3d_destroy_config(_robot, qcur);
// 
//   p3d_copy_config_into(_robot, qgrasp, qg);
//   p3d_copy_config_into(_robot, qpregrasp, qprev);
// 
//   p3d_destroy_config(_robot, qgrasp);
//   p3d_destroy_config(_robot, qpregrasp);
// //   XYZ_ENV->cur_robot= cur_robot;
...
//   }
// 
//   qcur = p3d_get_robot_config(_robot);
//   qpreplacement = p3d_alloc_config(_robot);
//   qplacement = p3d_alloc_config(_robot);
// 
//   deactivateCcCntrts(_robot, -1);
//   result = gpFind_placement_from_base_configuration(_robot, _object, _placementOnSupportList, GP_PA10, qcur, _grasp.at(armId), _handProp.at(armId), distance, qpreplacement, qplacement, _placement);
// 
//   if (result == GP_ERROR) {
...
//     p3d_set_and_update_this_robot_conf(_robot, qcur);
//     p3d_destroy_config(_robot, qcur);
//     p3d_destroy_config(_robot, qpreplacement);
//     p3d_destroy_config(_robot, qplacement);
//     return 1;
//   }
// 
// 
// 
//   p3d_set_and_update_this_robot_conf(_robot, qplacement);
//   getArmQ(q1, q2, q3, q4, q5, q6);
// 
//   p3d_set_and_update_this_robot_conf(_robot, qpreplacement);
// //   gpOpen_hand(robotPt, hand);
...
//   hand_robot = p3d_get_robot_by_name(hand_robot_name);
// 
//   cur_robot = XYZ_ENV->cur_robot;
//   XYZ_ENV->cur_robot = robotPt;
// 
...
//   configPt q0 = NULL;
// 
//   q0 = p3d_get_robot_config(_robot);
// 
// 
...
//   qbase = p3d_get_robot_config(_robot);
//   nb_iters_max = 300;
//   for (i = 0; i < nb_iters_max; i++) {
//     if (i != 0) {
//       qbase = gpRandom_robot_base(_robot, GP_INNER_RADIUS, GP_OUTER_RADIUS, objectCenter);
//       if (qbase == NULL) {
//         break;
//       }
//     }
//     p3d_set_and_update_this_robot_conf(_robot, qbase);
//     gpGet_platform_configuration(_robot, x0, y0, theta0);
//     p3d_set_and_update_this_robot_conf(_robot, q0);
//     gpSet_platform_configuration(_robot, x0, y0, theta0);
// 
//     if (p3d_col_test_robot(_robot, 0)) {
// 
//       continue;
//     }
//     g3d_draw_allwin_active();
...
//   }
// 
//   _cameraJnt = p3d_get_robot_jnt_by_name(_robot, (char*)cameraJntName);
// 
//   if (_cameraJnt == NULL)  {
...
//     p3d_multiLocalPath_disable_all_groupToPlan(_robot);
//     p3d_multiLocalPath_set_groupToPlan(_robot, _UpBodyMLP, 1) ;
// //    p3d_multiLocalPath_set_groupToPlan_by_name(_robot, (char*)"jido-arm_lin", 1) ;
//     deactivateCcCntrts(_robot, -1);
//   } else {
//     /* plan in the cartesian space */
//     qi = p3d_alloc_config(_robot);
...
//     p3d_destroy_config(_robot, qf);
//     if (_robot->nbCcCntrts != 0) {
//       p3d_activateCntrt(_robot, _robot->ccCntrts[0]);
//     }
//   }
...
// //   returnValue  = checkCollisionsOnPathAndReplan(_robot, traj, _robot->GRAPH, optimized);
//     traj = _robot->tcur;
...
//     return armPlanTask(ARM_FREE, 0, currentConfig, _robot->ROBOT_GOTO, (char*)"", lp, positions, segments);
//   }
// 
//   if (optimized && j > 1) {
