...
void p3d_randconfs(int NMAX, int (*fct_stop)(void), void (*fct_draw)(void)) {
  ...
  while (inode < p3d_get_NB_NODES()) {
    // ######### CASES #########
    ADDED = p3d_generate_random_free_conf(G, fct_stop, &fail);
...
void p3d_learn(int NMAX, int (*fct_stop)(void), void (*fct_draw)(void)) {
  ...
  inode = p3d_get_nb_ikSol(G->rob->cntrt_manager);
  fail = MY_ALLOC(int, inode);
  for (i = 0; i < p3d_get_nb_ikSol(G->rob->cntrt_manager); i++) {
    fail[i] = 1;
  }
  p3d_fail_stop(fail, 0, 1);//init
  inode = 0;
  while (inode < NMAX) {
  ...
  MY_FREE(fail, int, p3d_get_nb_ikSol(G->rob->cntrt_manager));
  p3d_fail_stop(fail, 0, -1);//desalloc
...
/**********************************************************/
/* Fonction creant un roadmap reliant deux configurations */
/* donnees                                                */
...
int p3d_specific_learn(double *qs, double *qg, int *iksols, int *iksolg, int (*fct_stop)(void), void (*fct_draw)(void)) {
  ...
  while (graph_node) {
    ...
    if (p3d_equal_config(robotPt, qs, qtest))
      Ns = graph_node->N;
    if (p3d_equal_config(robotPt, qg, qtest))
      Ng = graph_node->N;
    if (Ns && Ng) {
      //   Ns->radius = P3D_HUGE;
      Ns->radius = p3d_get_LAMBDA() * dmax;
      Ns->boundary = FALSE;
      //  Ng->radius = P3D_HUGE;
      Ng->radius = p3d_get_LAMBDA() * dmax;
      Ng->boundary = FALSE;
      break;
    }
  ...
  if (p3d_get_SAMPLING_CHOICE() == P3D_DYNAMIC_DOMAIN_SAMPLING) {
    /*we initialize the large box for random source */
    if ((Ns->comp->box_env_small)[0] == NULL) {
      (Ns->comp->box_env_small)[0] = p3d_copy_config(G->rob, Ns->q);
    }
    if ((Ns->comp->box_env_small)[1] == NULL) {
      (Ns->comp->box_env_small)[1] = p3d_copy_config(G->rob, Ns->q);
    }
    if ((Ng->comp->box_env_small)[0] == NULL) {
      (Ng->comp->box_env_small)[0] = p3d_copy_config(G->rob, Ng->q);
    }
    if ((Ng->comp->box_env_small)[1] == NULL) {
      (Ng->comp->box_env_small)[1] = p3d_copy_config(G->rob, Ng->q);
    }
    p3d_resize_rrt_box(G->rob, Ns->comp->box_env_small, Ns, 1);
    p3d_resize_rrt_box(G->rob, Ng->comp->box_env_small, Ng, 1);
  }
  ...
  while ((Ns->numcomp != Ng->numcomp) && !p3d_compco_linked_to_compco(Ns->comp, Ng->comp)) {
    switch (p3d_get_MOTION_PLANNER()) {
      ...
      case P3D_ISOLATE_LINKING:
        ...
        break;
      case P3D_RRT:
        ADDED = p3d_expand_start_rrt(G, fct_stop);
        break;
      case P3D_RRT2:
        ADDED = p3d_expand_bidir_rrt(G, fct_stop);
        break;
        //   case P3D_ADAPTATIVE_RRT:
        // ADDED = p3d_expand_start_rrt(G, fct_stop);
        // break;
#ifdef ENERGY
      case BIO_COLDEG_RRT:
...
#endif
      case P3D_LIGAND_RRT:
        ADDED = bio_start_lig_sc_rrt(G, fct_stop);
      case P3D_ALL_PRM:
        ADDED = p3d_add_all_prm_node(G, fct_stop);
        break;
      case P3D_FILTER_RRT:
        ADDED = bio_start_ligandbased_rrt(G, fct_stop);
        break;
  ...
  /*   if(p3d_get_MOTION_PLANNER() ==  P3D_ADAPTATIVE_RRT) { */
  /*     p3d_destroy_config(G->rob,(Ns->comp->box_env)[0]);  */
  /*     p3d_destroy_config(G->rob,(Ns->comp->box_env)[1]);  */
  /*     p3d_destroy_config(G->rob,(Ng->comp->box_env)[0]);  */
  /*     p3d_destroy_config(G->rob,(Ng->comp->box_env)[1]);  */
  /*   } */
  return(ADDED);
}
// fmodif Juan
 ...
 *****************************************************/
int p3d_generate_random_free_conf(p3d_graph *G, int (*fct_stop)(void), int * fail) {
  p3d_node *N = NULL;
  ...
  if (N == NULL) {
    if (p3d_col_get_mode() == p3d_col_mode_bio) {
      p3d_generate_random_free_conf(G, fct_stop, fail);
    } else
      return(FALSE);
  } else {
    p3d_insert_node(G, N);
...
int p3d_add_isolate_or_linking_node(p3d_graph *G, int (*fct_stop)(void),
  ...
  int loop = 0, test = 0, i = 0, j = 0, ikLayer = 0;
  //end path deform
  ...
  while (!ADDED) {
    ...
    for (i = 0; i < nbNodes; i++) {
      ...
      ikLayer = p3d_get_ik_choice() != IK_NORMAL ? p3d_get_ikSol_num(G->rob->cntrt_manager, (N[i])->iksol): 0;
      for(j = 0; j < p3d_get_nb_ikSol(G->rob->cntrt_manager); j++){
        if(j == ikLayer){//debug
          printf("--> ");
        }
        printf("%d\n", fail[j]);
...
//         PrintInfo(("time:\n"));
        ChronoPrint("");
//         PrintInfo(("\n"));
        fail[ikLayer] = 1;//reset the nb_try
        ADDED++;
      } else {//if LINKING node and can connect to other nodes
        fail[ikLayer]++;

        if (p3d_fail_stop(fail, ikLayer, 0)) {//if we reach the max number of Ntry stop
        ...
        if (NB_LINK > 1 && type != ISOLATED) {//if is it not isolated and have more than a link Connector
          if (G->oriented) {
            ...
            N[i]->type = LINKING;//is a linkin node
            //start path deform
            N[i]->only_for_cycle = FALSE;//is it not for cycles
            //end path deform
            ...
            while (node) {//for all linked nodes to the new one
              if (N[i]->isSingularity) {//debug
                printf("distance S = %d, N = %d : %f\n", N[i]->num, node->N->num, p3d_APInode_dist_multisol(G, N[i], node->N));
                printf("N[i]\n");
                p3d_print_iksol(G->rob->cntrt_manager, N[i]->iksol);
                printf("N\n");
                p3d_print_iksol(G->rob->cntrt_manager, node->N->iksol);
              }//debug
          ...
          ADDED++;
          p3d_print_iksol(G->rob->cntrt_manager, (N[i])->iksol);
          PrintInfo(("linking reset at : fail : %d\n", fail[ikLayer]));
          ChronoPrint("");
          fail[ikLayer] = 1;//reset the nb_try
        } else {
//           ADDED = FALSE;
      ...
      if ((p3d_get_cycles() == TRUE) && (ADDED == FALSE)) {
        ...
        if (test == TRUE) {
          ...
          if (ADDED == FALSE) {//ne sert a rien car si on rentre ici le noueud n'est pas ajouté ...
            p3d_insert_node(G, N[i]);
            N[i]->type = LINKING;
            N[i]->only_for_cycle = TRUE;
          }
          if (!p3d_is_node_in_list(N[i]->neighb, node1Pt)) {
            if (!p3d_APInode_linked_multisol(G, N[i], node1Pt, &dist)) {
          ...
          if (!p3d_is_node_in_list(N[i]->neighb, node2Pt)) {
            if (!p3d_APInode_linked_multisol(G, N[i], node2Pt, &dist)) {
