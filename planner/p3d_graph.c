...
void p3d_learn(int NMAX, int (*fct_stop)(void), void (*fct_draw)(void)) {
  ...
  if (p3d_get_multiGraph()) {
    ...
    }while((!p3d_doIncrementalConstruction(-1) && addedTab[mgNum] >= p3d_get_NB_TRY()) || (p3d_doIncrementalConstruction(-1) && !p3d_graph_to_traj(final->rob)));
      p3d_setSelectedMgNum(mgNum);
