...
p3d_multilocalpath_switch_to_linear_groups (p3d_rob * robotPt) {
  ...
  for (unsigned int u = 0; u < robotPt->armManipulationData->size (); u++) {
    if ( (*robotPt->armManipulationData) [u].getCartesian ()) {
      p3d_multiLocalPath_set_groupToPlan (robotPt,
                                          (*robotPt->
                                           armManipulationData) [u].
                                          getCartesianGroup (), 1);
    }
    p3d_multiLocalPath_set_groupToPlan (robotPt,
                                        (*robotPt->armManipulationData) [u].
                                        getHandGroup (), 1);
  }
  return 0;
}
...
//       }
  ...
  while (localpathMlp1Pt != NULL
    ...
    if (localpathMlp1Pt->next_lp == NULL
      ...
      localpathTransPt =
        p3d_softMotion_localplanner (robotPt, IGRAPH_OUTPUT,
                                     softMotion_data_lpTrans, q1, q2, q2,
                                     localpath1Pt->ikSol);
      ...
      if (localpathTransPt != NULL) {
        collision =
          p3d_unvalid_localpath_test (robotPt, localpathTransPt, ntest);
      }
      if (localpathTransPt == NULL || collision == TRUE) {

        printf
        ("localpathTransPt : stop motion localpath = %p , collision = %d\n",
...
p3d_convert_traj_to_softMotion (p3d_traj * trajPt, bool param_write_file,
  ...
  trajSmPTPPt->courbePt =
    p3d_local_planner_multisol (robotPt, q_init, q_end,
                                localpathMlp1Pt->ikSol);


  if (trajSmPTPPt->courbePt->activeCntrts != NULL) {
    MY_FREE (trajSmPTPPt->courbePt->activeCntrts, int,
             trajSmPTPPt->courbePt->nbActiveCntrts);
  }
  trajSmPTPPt->courbePt->nbActiveCntrts = localpathMlp1Pt->nbActiveCntrts;
  trajSmPTPPt->courbePt->activeCntrts =
    MY_ALLOC (int, trajSmPTPPt->courbePt->nbActiveCntrts);
  for (int v = 0; v < localpathMlp1Pt->nbActiveCntrts; v++) {
    trajSmPTPPt->courbePt->activeCntrts[v] =
      localpathMlp1Pt->activeCntrts[v];
  }
  ...
  while (localpathMlp1Pt != NULL) {
    ...
    if (localpath1Pt != NULL) {
      q_init =
        localpathMlp1Pt->config_at_param (robotPt, localpathMlp1Pt, 0.0);
      q_end =
        localpathMlp1Pt->config_at_param (robotPt, localpathMlp1Pt,
                                          localpathMlp1Pt->length_lp);
      ...
      for (int v = 0; v < localpathMlp1Pt->nbActiveCntrts; v++) {
        localpathTmp1Pt->activeCntrts[v] =
          localpathMlp1Pt->activeCntrts[v];
      }

      TRAJPTP_CONFIG[NB_TRAJPTP_CONFIG] =
        p3d_copy_config (robotPt, q_end);
      p3d_destroy_config (robotPt, q_init);
      p3d_destroy_config (robotPt, q_end);
      NB_TRAJPTP_CONFIG++;
