...
int
    ...
    p3d_multiLocalPath_disable_all_groupToPlan (robotPt, 0);
    for (int iGraph = 0; iGraph < robotPt->mlp->nblpGp; iGraph++) {
        if (strcmp (robotPt->mlp->mlpJoints[iGraph]->gpName, "upBodySm") == 0) {
            p3d_multiLocalPath_set_groupToPlan (robotPt, iGraph, 1, 0);
                                        ...
                                        double > > &positions, SM_TRAJ & smTraj)
{
    ...
    // initialize the index of localpath
    int lpId = 1;
...
#endif
    /////////////////////////////////////////////////////////////////////////////////
    ////  CONVERT LINEAR TRAJECTORY TO SOFTMOTION POINT TO POINT TRAJECTORY (PTP) ///
    /////////////////////////////////////////////////////////////////////////////////
    ...
    // end of ptp
    clock_end = clock();
    cout << "SM ptp = " << ((double)clock_end - clock_start) / CLOCKS_PER_SEC << " s ##########################"<< endl;
#endif
    ...
    if(smooth == false)
    {
        ...
        p3d_softMotion_export_traj (robotPt, trajSmPTPPt, 0,
                                    ...
                                    (char *) "softMotion_PTP_Seg.traj",param_write_file,
                                    ENV.getBool (Env::plotSoftMotionCurve), SAMPLING_TIME, lp,
                                    positions, smTraj);
#ifdef DEBUG_STATUS
        // end of exporting
        clock_end = clock();
        cout << "SM exporting ptp = " << ((double)clock_end - clock_start) / CLOCKS_PER_SEC << " s ##########################"<< endl;
#endif
    } else {
        ...
        // end of smooth
        clock_end = clock();
        cout << "SM smooth = " << ((double)clock_end - clock_start) / CLOCKS_PER_SEC << " s ##########################"<< endl;
#endif

        /* Write curve into a file for BLTPLOT *///ENV.getBool(Env::plotSoftMotionCurve)
#ifdef DEBUG_STATUS
        // timer for exporting smooth
        clock_start = clock();
#endif
        smTraj.clear ();
        p3d_softMotion_export_traj (robotPt, trajSmPt, 1,
                                    (char *) "softMotion_Smoothed_Q.traj",
                                    (char *) "softMotion_Smoothed_Seg.traj",param_write_file,
                                    ENV.getBool (Env::plotSoftMotionCurve), SAMPLING_TIME, lp,
                                    positions, smTraj);
#ifdef DEBUG_STATUS
        // end of exporting
        clock_end = clock();
        cout << "SM exporting smooth = " << ((double)clock_end - clock_start) / CLOCKS_PER_SEC << " s ##########################"<< endl;
#endif
    }
...
#ifdef DEBUG_STATUS
        // end of approxi;ation
        clock_end = clock();
        cout << "SM approximation = " << ((double)clock_end - clock_start) / CLOCKS_PER_SEC << " s ##########################"<< endl;
#endif
    }
